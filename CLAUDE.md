...
**World Query is not Depth Module.** `DepthModule` genuinely streams nothing in Preview; `WorldQueryModule` does NOT share that limitation — the docs list "Interactive Preview enabled" as a supported configuration and require `EXPERIMENTAL_API` only for semantic classification (`options.classification`), which `PotPlacement` does not use. An earlier conclusion here conflated the two and cast the placement ray straight DOWN from the pot, three metres out of the camera frustum, where no depth exists by definition. The ray now sweeps forward-and-down inside the viewed region and retries across frames. That is the correct implementation, but it still returned no hit across two diagnostic runs in the "Evening Room" preview scene, so the fixed 70cm/75cm fallback remains what a Preview recording shows. Whether a real surface answers is **untested on device** — do not assume either way from the Preview result.
